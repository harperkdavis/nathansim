...
function unlockSkin(nathan) {
  NSG.unlockedNathans.push(nathan);
  saveSave();
}
...
function unlockAchievement(achievement) {
  ...
  NSG.unlockedAchievements.push(achievement.name);
  unlockSkin(achievement.reward);
  saveSave();
}

function incrementAchievement(achievement) {
  if (!d5340dbe53de7c8912916a41d074ea92 || achievement.name == "Filthy Cheater") {
    if (!NSG.unlockedAchievements.includes(achievement.name)) {
      if (Object.keys(NSG.achievementProgress).includes[achievement.name]) {
        NSG.achievementProgress[achievement.name] += 1;
      } else {
        NSG.achievementProgress[achievement.name] = 1;
      }
      if (NSG.achievementProgress[achievement.name] >= achievement.max) {
...
let achievements = [
  {
    name: "Secret Rufus Achievement",
    desc: "Activate Rufus Mode.",
    reward: "rufus",
    max: 1,
    condition: () => {
      return currentNathan == 'rufus';
    }
  },
  {
    name: "Filthy Cheater",
    ...
    max: 1,
    condition: () => {
      return d5340dbe53de7c8912916a41d074ea92;
    }
  },
  {
    name: "Filthy Rich",
    desc: "Have over a trillion dollars.",
    reward: "rich",
    max: 1,
    condition: () => {
  ...
  {
    name: "Backpack Recovered",
    desc: "Beat the game.",
    reward: "cool",
    max: 1,
    condition: () => {
      return wonTheGame && frameCount == stopTime + 1;
    }
  },
  {
    name: "What?",
    desc: "Beat the game without upgrades.",
    reward: "upside down",
    max: 1,
    condition: () => {
      return wonTheGame && stats['upgrades purchased'] == 0;
    }
  },
  {
    name: "Investor",
    ...
    max: 1,
    condition: () => {
      return wonTheGame && stats['enemies killed'] == 1;
    }
  },
  {
    name: "Gunslinger",
    desc: "Beat the game with only a pistol.",
    reward: "cowboy",
    max: 1,
    condition: () => {
      return wonTheGame && ownedGuns.length == 1;
    }
  },
  {
    name: "Nerd",
    desc: "Stop procrastinating.",
    reward: "nerd",
    max: 1,
    condition: () => {
      return false;
  ...
  {
    name: "The Real Nathan Chen",
    desc: "Beat the game 100 times.",
    reward: "etherial",
    max: 100,
    condition: () => {
      return wonTheGame && frameCount == stopTime + 1;
...
function loadSave() {
  let lastSave = localStorage.getItem("nsgData");
  if (lastSave == null) {
    saveSave();
...
function setup() {
  ...
  fetch("https://protected-reef-87558.herokuapp.com/top5/").then(res => res.json()).then(json => {top5 = json});
  ...
  f6d62567f569f58d50984e9cf2e9e381 = true;
  c21f8c99d751dc28802fd7c8ebe7ce4a = true;

  cutscene.transBackpack = loadImage("https://i.imgur.com/8gCLOCM.png");

  nathans['standard'] = loadImage("https://i.imgur.com/qNRBIvl.png");
  nathans['rufus'] = loadImage("https://i.imgur.com/ehb9nsw.png");
  nathans['cheater'] = loadImage("https://i.imgur.com/OLqjYzh.png");
  nathans['cowboy'] = loadImage("https://i.imgur.com/EpI3ToY.png");
  nathans['cool'] = loadImage("https://i.imgur.com/vdkbLyC.png");
  nathans['etherial'] = loadImage("https://i.imgur.com/SQyfmcR.png");
  nathans['investor'] = loadImage("https://i.imgur.com/9NCXGsX.png");
  nathans['rich'] = loadImage("https://i.imgur.com/EpB0Nsd.png");
  nathans['nerd'] = loadImage("https://i.imgur.com/kEcMlYW.png");
  nathans['upside down'] = loadImage("https://i.imgur.com/mVz6kgC.png");
...
function keyPressed() {
  ...
  if (keylogger.length >= 5) {
    if (keylogger[keylogger.length - 5] == 'r' &&
      keylogger[keylogger.length - 4] == 'e' &&
      keylogger[keylogger.length - 3] == 's' &&
      keylogger[keylogger.length - 2] == 'e' &&
      keylogger[keylogger.length - 1] == 't') {
        localStorage.clear();
      }
  }
...
function draw() {
  ...
  if (inMainMenu) {
    ...
    imageMode(CENTER);
    image(nathans[currentNathan], width / 2, height / 2 + sin(frameCount * 0.06) * 10, 128, 128);
    ...
    rectMode(CORNER);
    let j = 0;
    Object.keys(nathans).forEach(nKey => {
      image(nathans[nKey], 10, 100 + j * 70, 64, 64);
      fill(0);
      noStroke();
      textAlign(LEFT, TOP);
      textStyle(BOLD);
      textSize(20);
      text(toTitleCase(nKey), 80, 100 + j * 70);

      if (!NSG.unlockedNathans.includes(nKey)) {
        achievements.forEach(ach => {
          if (ach.reward == nKey) {
            textStyle(NORMAL, ITALIC);
            text('Complete "' + ach.name + '"', 80, 100 + j * 70 + 20);
          }
        });
        fill(100, 100);
        rect(8, 98 + j * 70, 420, 68);
      } else {
        if (mouseIsPressed && isWithin(mouseX, mouseY, 8, 98 + j * 70, 420, 68)) {
          currentNathan = nKey;
        }
      }
    ...
    achievements.forEach(ach => {
      ...
      textSize(16);
      textStyle(BOLD);
      let prog = Object.keys(NSG.achievementProgress).includes(ach.name) ? NSG.achievementProgress[ach.name] : 0;
      text(ach.name + " (" + (prog / ach.max * 100) + "%)", width - 4, 200 + k * 50);
      textStyle(NORMAL);
      text(ach.desc, width - 4, 200 + k * 50 + 20);
      k++;
    });
  ...
  imageMode(CORNERS);
  image(nathans[currentNathan], -32 * (1 / nathanHeight) + position.x, position.y, 33 * (1 / nathanHeight) + position.x, -64 * nathanHeight + position.y);
  ...
  if (wonTheGame) {
    ...
    if (frameCount > stopTime + 600 && fetchHttpStatus == false) {
      let name = (keylogger[keylogger.length - 6][0] + keylogger[keylogger.length - 5][0] + keylogger[keylogger.length - 4][0] + keylogger[keylogger.length - 3][0] + keylogger[keylogger.length - 2][0] + keylogger[keylogger.length - 1][0]).toUpperCase();
      name = name.replace(" ", "L");
      if (frameCount < stopTime + 1000) {
        textAlign(CENTER, CENTER);
        textStyle(BOLD);
        text("Enter your name: ", 200, 450);

        textStyle(NORMAL);
        for (let i = 0; i < 6; i++) {
          text(name[i], 160 + i * 15, 480);
        }

        text("Score sending in " + (1000 - (frameCount - stopTime)), 200, 520);
      } else if (frameCount == stopTime + 1000) {
        sentData = {
          "name": name,
          "time": time
        };
        fetch('https://protected-reef-87558.herokuapp.com/newScore', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify(sentData),
        }).then(res => res.json()).then(json => winData = json);
      }
      if (frameCount >= stopTime + 1000) {
        textAlign(CENTER, CENTER);
        text("Thanks, " + sentData.name + "!", 200, 450);
        if (winData["rank"] != undefined) {
          text("You are global rank #" + winData["rank"] + "!", 200, 520);
        } else {
          text("Processing...", 200, 520);
        }
        
      }
    }

    if (frameCount > stopTime + 1100) {
      textAlign(CENTER, CENTER);
      textStyle(BOLD);
      text("but can you do better?", 200, height - 200);
    }

    if (frameCount > stopTime + 1200) {
      textAlign(CENTER, CENTER);
      textStyle(NORMAL);
